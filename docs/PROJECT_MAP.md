...
- **Phase 3 — DESIGNED 2026-07-28, code starts next session.** Five sub-phases, one per session: **3a** CI + skeleton + security baseline (the pipeline is the deliverable — D60; first-ever `.github/`, and the Android build has *never* run in CI) → **3b** models + migrations → **3c** auth → **3d** sync + `/me/today` → **3e** client queue. **3a–3d touch zero `app/` files**, so the shipped offline app cannot regress. Decisions recorded: **D59** monorepo, **D60** security-first + pipeline-as-deliverable, **D65** reconciliation (`server_acked + local_unacked`, CRDT-shaped), **D66** Redis/FCM deferred to P4. **D61–D64 deliberately unwritten** — cold starts, pooled-vs-direct URLs, refresh rotation, day boundary are claims about real infra and get recorded in the sub-phase that verifies them.
